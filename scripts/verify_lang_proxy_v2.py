...
from tcg_platform.scraping.ebay import parse_ebay_item_page, _split_base_version

SOURCE_DB = "data/tcg.db"
VERIFY_DB = "data/tcg_v2.db"
MAX_WORKERS = 8
SCRAPE_DELAY = 0.5
...
def scrape_one(client, card_id: str, url: str, current_lang: str) -> dict:
    ...
    try:
        ...
        result["new_card_version"] = rec.card_version
        result["new_lang"] = rec.language
        result["is_proxy"] = rec.card_id.startswith("proxy_")
        result["price"] = rec.price

        mismatch = (
            result["new_lang"] != current_lang
            or result["is_proxy"]
            or result["new_card_id"] != card_id
        )
...
def main():
        ...
        for future in as_completed(futures):
            ...
            if res["status"] == "mismatch":
                ...
                print(f"[{completed}/{len(urls)}] MISMATCH {res['item_id']}: "
                      f"lang={res['old_lang']}→{res['new_lang']} "
                      f"card={res['old_card_id'][:30]}→{res['new_card_id'][:30]} "
                      f"ver={res['new_card_version'] or ''}")
            elif res["status"] == "ok":
                print(f"[{completed}/{len(urls)}] OK {res['item_id']} lang={res['new_lang']} "
                      f"card={res['new_card_id'][:35]} ver={res['new_card_version'] or ''}")
            else:
    ...
    lang_rows = verify_conn.execute("SELECT language, COUNT(*) FROM fact_events GROUP BY language").fetchall()
    print("Verify DB languages:", {r["language"]: r["COUNT(*)"] for r in lang_rows})

    # Show card_version distribution
    version_rows = verify_conn.execute(
        "SELECT card_version, COUNT(*) FROM fact_events GROUP BY card_version ORDER BY COUNT(*) DESC LIMIT 10"
    ).fetchall()
    print("\nTop card_versions:")
    for r in version_rows:
        print(f"  '{r['card_version'] or '(empty)'}: {r['COUNT(*)']} rows")

    # Show base card_id (without version) uniqueness
    base_ids = verify_conn.execute("SELECT DISTINCT card_id FROM fact_events").fetchall()
    print(f"\nDistinct base card_ids: {len(base_ids)}")
    verify_conn.close()

    with open("data/verify_v2_mismatches.json", "w") as f:
        json.dump(mismatches, f, indent=2, default=str)
    print(f"\nFull mismatch list → data/verify_v2_mismatches.json")
