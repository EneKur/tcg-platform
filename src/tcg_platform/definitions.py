...
ebay_de_job = define_asset_job(
    ...
    selection=["ebay_de_sold_listings", "bronze_ebay_de_sqlite_writer", "backfill_de_sold_data_parquet"],
    description="Scrape DE eBay sold listings, persist to SQLite + MinIO parquet",
)

ebay_uk_job = define_asset_job(
    name="ebay_uk_pipeline",
    selection=["ebay_uk_sold_listings", "bronze_ebay_uk_sqlite_writer", "backfill_uk_sold_data_parquet"],
    description="Scrape UK eBay sold listings, persist to SQLite + MinIO parquet",
)

ebay_eu_job = define_asset_job(
    name="ebay_eu_pipeline",
    selection=["ebay_de_sold_listings", "bronze_ebay_de_sqlite_writer", "backfill_de_sold_data_parquet",
               "ebay_uk_sold_listings", "bronze_ebay_uk_sqlite_writer", "backfill_uk_sold_data_parquet"],
    description="Scrape DE+UK eBay sold listings, persist to SQLite + MinIO parquet",
)
