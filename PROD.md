...
- **A hung Zyte request will block the scraper forever** (no per-call timeout in the Zyte SDK by default). Symptom: the `scrape_ebay_*_raw` step shows no log output for tens of minutes; `lsof -p <pid>` shows an `ESTABLISHED` TCP to `69.41.180.81:443`. Fixed in PR #20 + #23: `ZyteSessionResource` now creates a shared `aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=ZYTE_API_TIMEOUT))` (default **60s**, override via env) AND applies a hard Python-level `future.result(timeout=ZYTE_API_TIMEOUT)` on every Zyte call. The single key is managed externally; the scraper wraps each Zyte call in a `try/except` so a hung connection raises `ZyteTimeoutError`, the scraper logs `STOP ... exc=ZyteTimeoutError` and continues — it does **not** crash the whole pipeline.
